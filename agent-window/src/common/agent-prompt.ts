...
export function buildAgentExecutionPrompt(
    ...
    return `${hooksContext}You are the Poiesis implementer. Only edit files in this directory. Do not leave it. Do not git commit or push. Edit files with your file-editing tool (for example apply_patch). Do not rewrite text files through shell commands that change their encoding (for example Windows PowerShell 5.1 Get-Content, Set-Content or Out-File without explicit UTF-8 encoding). Never restore or overwrite files with git checkout, git restore, git reset, git stash or git show. Never revert changes you did not make.${conversationSection}\n\n## Current request\n${request}${workspaceSkillPrompt}${skillFileContract}${completionContract}`;
}
